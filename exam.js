...
function mostrarUsuarios() {
    ...
    for (var i = 0; i < users.length; i++){
      ...
      elementoLi.textContent = "Nombre: " + users[i].nombre + " - " + users[i].edad + " años" + " Teléfono: " + users[i].pos;
      
      // Agregar datos de checkbox
      //elementoLi.textContent += " - Complementos: " + users[i].complementos.join(", ");
      // Agregar datos de radiobutton
      elementoLi.textContent += " - Editorial: " + users[i].estadio;
      // Agregar datos del select
      elementoLi.textContent += " - Campus: " + users[i].equipo;
...
function displayUsers() { 
    ...
    users.forEach((user, index) => {
        ...
        const cellEstadio = row.insertCell(3);
        // Inserta una celda para el equipo del usuario en la fila creada
        const cellEquipo = row.insertCell(4);
...
document.getElementById('addForm').addEventListener('submit', function(event) {
    ...
    if (confirmacion) { 
        ...
        users.push({ nombre, edad, pos, estadio, equipo });
        displayUsers();
        mostrarUsuarios();
...
function editUser(index) {
    ...
    // Radiobuttons
    document.getElementById('estadioOlimpico').checked = user.estadio === 'Patria';
    document.getElementById('estadioSanJorge').checked = user.estadio === 'McGrill';
    document.getElementById('estadioColima').checked = user.estadio === 'Otra';
    // Select
